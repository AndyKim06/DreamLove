...
class PromptBuilder:
    ...
    def build_prompt(gender: str, animal: str, eyelid: str, face_shape: str,
        ...
        # 한글 요약 생성
        animal_kor = {
            "puppy": "강아지상", "cat": "고양이상", "deer": "사슴상", 
            "rabbit": "토끼상", "dinosaur": "공룡상", "fox": "여우상", "bear": "곰상"
        }
        gender_kor = {"male": "남성", "female": "여성"}
        eyelid_kor = {"monolid": "무쌍", "double": "쌍커풀"}
        makeup_kor = {"light": "연한", "heavy": "진한"}
        face_kor = {"v-line": "V라인", "round": "둥근형"}
        length_kor = {"long": "긴머리", "short-bob": "단발", "short": "숏컷"}
        
        korean_summary = (
            f"{gender_kor.get(gender, gender)}_"
            f"{animal_kor.get(animal, animal)}_"
            f"{eyelid_kor.get(eyelid, eyelid)}_"
            f"{face_kor.get(face_shape, face_shape)}_"
            f"{length_kor.get(hairstyle, hairstyle)}_"
            f"{makeup_kor.get(makeup, makeup)}"
        )
        
        return final_prompt, final_negative, korean_summary, tokens